...
public class SlashCmdEventManager implements SlashCommandCreateListener {
    ...
    public SlashCmdEventManager(DiscordApi api, SlashCmd cmd) {
        ...
        SlashCommandBuilder scb = new SlashCommandBuilder().setName(cmd.getName()).setDescription(cmd.getDescription());
        ...
        cmd.getServer().ifPresentOrElse(server -> {
            cmdId.append(scb.createForServer(server).join().getIdAsString());
            cmd.getPermissions().ifPresent(perms -> {
                new SlashCommandPermissionsUpdater(server).addPermissions(perms)
                        .update(Long.parseLong(cmdId.toString())).join();
            });
        }, () -> {
